...
- Dashboard remove confirmation is now a centered blocking popup
  (lazydocker-style, like the `?` menu): `x`/`X` opens a `Confirm
  remove` / `Confirm remove --force` box naming the target worktrees
  with a `y/n` hint, hiding the grid panes until `y` confirms or
