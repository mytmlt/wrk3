...
```bash
curl -fsSL https://raw.githubusercontent.com/mytmlt/wrk3/main/scripts/install.sh | bash -s -- --version v0.8.0
...
Env overrides: `WRK3_VERSION` (default `latest`), `PREFIX`/`BINDIR`
(install dir), `WRK3_VERIFY=0` (same as `--no-verify`).
...
```bash
# linux example (tag v0.8.0 -> file wrk3_0.8.0_...)
curl -fsSLO https://github.com/mytmlt/wrk3/releases/download/v0.8.0/wrk3_0.8.0_linux_amd64.tar.gz
curl -fsSLO https://github.com/mytmlt/wrk3/releases/download/v0.8.0/checksums.txt
sha256sum -c <(grep wrk3_0.8.0_linux_amd64.tar.gz checksums.txt)
tar -xzf wrk3_0.8.0_linux_amd64.tar.gz wrk3
...
```text
A new version of wrk3 is available: v0.8.0 (you have v0.7.0). Run "wrk3 update" to update.
```
...
wrk3 completion zsh > ~/.config/zsh/completions/_wrk3
wrk3 completion fish > ~/.config/fish/completions/wrk3.fish
wrk3 completion powershell > wrk3.ps1
```
