...
2. **Register in `internal/runner/registry.go`** via the `init()` above.
   ...
   `Available()` (see `docs/USAGE.md` troubleshooting).
...
4. **Wire construction** in `cmd/common.go: newRunner`. The `docker` type
   is special-cased (`runner.New(cfg.ComposeOptions(slug))`); other
   backends go through their registered factory as-is. If your backend
   needs per-worktree options (like `slug`), extend that function with a
   `cfg.Runner.Type == "myrunner"` branch that builds your options from
   `cfg` + `slug` — still via the `runner` package API, never by
   importing your concrete type's internals beyond its constructor.
