...
- Each `add` takes the next index (`max(index)+1`, starting at 0) and
  upserts the allocation into the worktree's `.env` plus the state file.
  Upsert means: managed keys are updated in place when present, appended
  (under a `# Managed by wrk3` marker) when missing; every other line —
  secrets, comments, blank lines — passes through verbatim. Re-running is
  idempotent and never duplicates keys. Fresh worktrees with no `.env`
  get a generated one (`# Generated by wrk3` header).
...
- The repo-root main checkout is implicit (no state entry) with reserved
  ...
  managed `.env` keys are upserted on `up`/`down`/`exec`. Port/project
  collisions with managed worktrees surface as errors.
