...
func TestLocalBranches_ListsLocalOnly(t *testing.T) {
	repo := initRepo(t)
	src := &GitSource{}
	runGit(t, repo, "branch", "feature/local-only")
	got, err := src.LocalBranches(repo)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !found["main"] || !found["feature/local-only"] {
		t.Errorf("got %v, want main + feature/local-only", got)
	}
}

func TestParseLocalBranches_SkipsNoise(t *testing.T) {
	got := parseLocalBranches("main\n\nfeature/foo\n* stray*\n(no branch)\nmain\n")
	if len(got) != 2 || got[0] != "main" || got[1] != "feature/foo" {
		t.Errorf("got %v, want [main feature/foo]", got)
	}
}
